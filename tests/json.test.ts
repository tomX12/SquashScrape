import { dataToJson } from "../src/Scraper/DataScraper";
import * as fs from 'fs'

beforeEach(async () => {
    //if latestMatch.json exists delete it
    if (fs.existsSync("latestMatch.json")) {
        fs.unlinkSync("latestMatch.json");
    }
    await dataToJson()
})

test('adds json file successfully',  () => {
    // Check if the file exists
    const fileExists = fs.existsSync("latestMatch.json");
    // Assert that the file exists
    expect(fileExists).toBe(true);
})

test('should have the correct field names and value types', () => {
    const jsonData = JSON.parse(fs.readFileSync('latestMatch.json', 'utf-8'));
